...
Because `ny_noaa` is a massive dataset, best to work with a subset of small random sample from the dataset and only remove the code after everything is finalized. 
...
Because `prcp`, `tmax` and `tmin` were recorded in tenth of the desired units (mm or degrees C), we first convert them to the reasonable units of mm or degrees C. For snowfall, the most commonly observed values is `r names(sort(-table(ny_noaa_sam$snow)))[1]`. This is because for most of the year there is no or very minimal snowfall in the state of New York, and record will only have non-zero values in late fall and winter seasons when snowfall actually occurs. 
...
Try this on the entire dataset
